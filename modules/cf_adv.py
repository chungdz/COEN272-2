...
class CFAdvanced:
    ...
    def cf(self, testd: dict, k=100):
        ...
        for uid, uinfo in tqdm(testd.items()):
            ...
            for mid, rating in uinfo['rated'].items():
                uinfo['avg_rated'][mid] = rating - uinfo['avg']
                # only use movies that are reted before
                if mid in self.m:
                    uinfo['iuf_rate'][mid] = self.m[mid]['IUF'] * uinfo['rated'][mid]
            # calculate enhanced weights
            # enhanced weight = weight * weight^1.5
            wl = []
            for dbuid, dbuinfo in self.s.items():
                curw = self.cal_weights(dbuinfo, uinfo)
                if curw != 0:
                    wl.append([curw * math.pow(abs(curw), 1.5), dbuid])
            # sort weights
    ...
    def cal_weights(self, dbuinfo, testuinfo):
        intersect = []
        for mid in testuinfo['iuf_rate']:
            if mid in dbuinfo['iuf_rate']:
                intersect.append(mid)
        
        if len(intersect) == 0:
            return 0
        
        dbrate = [dbuinfo['iuf_rate'][mid] for mid in intersect]
        testrate = [testuinfo['iuf_rate'][mid] for mid in intersect]
